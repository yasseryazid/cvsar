...
@section('content')
    <div class="card card-body">
        <h4>Create</h4>
        <form action="{{ route('rooms.store') }}" onsubmit="Are you sure want to store this data?" method="post">
            @csrf
            ...
            <div class="form-group pt-2">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="slug" id="" class="form-control @error('slug') is-invalid @enderror"
                        placeholder="Input url without space (ex. executive-room)" value="{{ old('slug') }}">
                </div>
                @error('slug')
                    <div class="alert alert-danger">
                        {{ $message }}
